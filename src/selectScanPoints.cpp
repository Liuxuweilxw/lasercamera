...
//        std::cout << points.at(id).transpose() <<" "<<points.at(id+1).transpose() <<std::endl;
    ...
    // int id_left = std::min( id + delta, n-1);
    // std::cout<<id_left<<endl;
    // int id_right = std::max( id - delta , 0);
    // std::cout<<id_right<<endl;
    ...
    //             Eigen::Vector3d dist = points.at(seg.id_start) - points.at(seg.id_end);
    //             // if(dist.head(2).norm() > 0.2
    //             if(points.at(seg.id_start).head(2).norm() < 1.5
    //                && points.at(seg.id_end).head(2).norm() < 1.5)
    //             //    && seg.id_end-seg.id_start > 50   )  // 至少长于 20 cm, 标定板不能距离激光超过2m, 标定板上的激光点肯定多余 50 个
    //             {
    //                 seg.dist = dist.head(2).norm();
    //                 segs.push_back(seg);
    ...
    //     //     ptsLine.push_back(points.at(i));
    //     // }
    //     for (int i = bestLine.id_start; i < bestLine.id_end; ++i) {
    //     // for (int i = segs1.at(0).id_start; i < segs1.at(segs1.size()-1).id_end; ++i) {
    //         if(points.at(i)(0) >1.5){
    //             continue;
    //         }
    //         ptsLine.push_back(points.at(i));
    //     }
...
//                 Eigen::Vector3d dist = points.at(seg.id_start) - points.at(seg.id_end);
//                 if(dist.head(2).norm() > 0.2
//                 &&points.at(seg.id_start).head(2).norm() < 1.5
//                    && points.at(seg.id_end).head(2).norm() < 1.5
//                    && seg.id_end-seg.id_start > 50   )  // 至少长于 20 cm, 标定板不能距离激光超过2m, 标定板上的激光点肯定多余 50 个
//                 {
//                     seg.dist = dist.head(2).norm();
...
// std::vector< Eigen::Vector3d > AutoGetLinePts( std::vector<Eigen::Vector3d> points, bool debug)
// {
// //    cv::Mat img(img_w, img_w, CV_8UC1, cv::Scalar::all(0));
//     cv::Mat img(img_w, img_w, CV_8UC3, cv::Scalar(0,0,0));
...
//         cv::Vec3b color_value(255,0,0);                         //blue  BGR
//         img.at<cv::Vec3b>(row, col) = color_value;
// //        img.at<uchar>(row, col) = 255;
//     }
...
//     // std::cout<<"id_left : "<<id_left<<std::endl;
//     // std::cout<<"id_right : "<<id_right<<std::endl;

//     int dist_left = points.at(id_left).norm();
//     int dist_right = points.at(id_right).norm();

//     // std::cout<<"dist_left : "<<dist_left<<std::endl;
//     // std::cout<<"dist_right : "<<dist_right<<std::endl;

//     // 逻辑别搞复杂了。
//     std::vector<LineSeg> segs;
//     // std::vector<LineSeg> segs2;
//     // double dist_thre = 0.08;
//     // double dist_thre = 0.08;
//     int skip = 1;
//     int currentPt = id_right;
...
//      std::vector<Eigen::Vector3d> ptsLine;
//     for (int i = id_right; i < id_left - skip; i += skip) {
//         // std::cout<<"range: "<<points.at(i).head(2).norm()<<std::endl;
...
//             newSeg = false;
//         }
//         // std::cout<<"currentPt : "<<currentPt<<std::endl;
...
//         if(d1 < range_max )    // 有效数据,  激光小于 100 m
//         {

//             if(points.at(seg.id_start).head(2).norm()>range_max){
//                 seg.id_start = currentPt;
//             }

//             seg.id_end = currentPt;

//             // // std::cout<<"each points :"<<points.at();
//             // if(fabs(d1-d2) < dist_thre)  //  8cm
//             // {
//             //     seg.id_end = nextPt;
//             // } else
...
//             //     newSeg = true;
//             //     
//             //     // if(points.at(seg.id_start).head(2).norm() < 2
//             //     //    && points.at(seg.id_end).head(2).norm() < 2
...
//     // 对 segs1 的边界进行扩充
//     // int i=0;
//     // // std::cout<<"segs1's size is :"<<segs1.size()<<std::endl;
//     // for (i = 0; i < segs.size(); ++i) {
//     //     LineSeg tmp = segs.at(i);
//     //     // std::cout<<"echo  :"<< segs1.at(i).id_start <<std::endl;
//     //     // std::cout<<"tmp.id_start  : "<<tmp.id_start<<std::endl;
...
//     //     {
//     //         int boundaryPt = tmp.id_start + j;
//     //         // std::cout<<"boundaryPt : "<<boundaryPt<<std::endl;
//     //         // if(boundaryPt<0 )boundaryPt = 0; // Edit 2
//     //         double d1 = points.at(tmp.id_start).head(2).norm();
//     //         double d2 = points.at(boundaryPt).head(2).norm();
//     //         if(fabs(d1-d2) < dist_thre)  //  8cm
//     //         {
...
//     // std::cout<<"Segs's size: "<<segs1.size()<<std::endl;
//     // if(segs.size() > 0)
//     // {
//     //     LineSeg bestLine;
//     //     int maxpts = -1;
...
//     //         LineSeg tmp = segs.at(i);
//     //         // std::cout<<"tmp.id_start : "<<tmp.id_start<<std::endl;
//     //         // std::cout<<"tmp.id_end : "<<tmp.id_end<<std::endl;
...
//     //     // for (int i = bestLine.id_start; i < bestLine.id_end+1; ++i) {
//     //     for (int i = segs.at(0).id_start; i < segs.at(segs.size()-1).id_end; ++i) {
//     //         ptsLine.push_back(points.at(i));
//     //     }
//     // }
...
//        //the follow is to ensure  no  noverty points
//         for (int i = bestLine.id_start; i < bestLine.id_end; ++i) {
//         // for (int i = segs1.at(0).id_start; i < segs1.at(segs1.size()-1).id_end; ++i) {
//             if(points.at(i)(0) >2.0){
//                 continue;
//             }
...
//                 cv::Vec3b color_value(0,255,0);   //green BGR
//                 img.at<cv::Vec3b>(row, col) = color_value;
//             }
//         }
...
//             cv::Vec3b color_value(0,0,200);   //red BGR
//             img.at<cv::Vec3b>(row, col) = color_value;
//         }
