...
void CamLaserCalClosedSolution(const std::vector<Oberserve> obs, Eigen::Matrix4d &Tlc)
{
    ...
    */
    ...
    for (size_t i = 0; i < obs.size(); i++)
    {
        ...
        std::cout<<"(Tctag.inverse()).transpose() : "<<(Tctag.inverse()).transpose()<<std::endl;
        std::cout<<"Planar_cam : "<<planar_cam<<std::endl;
        ...
        for (size_t j = 0; j < calibra_pts.size(); ++j) 
        {
            ...
            // Ai<<nc.x() * bar_p.x(), nc.x() * bar_p.y(),  nc.x() * bar_p.z(),  
            //         nc.y() * bar_p.x(), nc.y() * bar_p.y(), nc.y() * bar_p.z(),  
            //         nc.z() * bar_p.x(), nc.z() * bar_p.y(), nc.z() * bar_p.z();
...
#define LOSSFUNCTION
void CamLaserCalibration1(const std::vector<Oberserve> obs, Eigen::Matrix4d &Tcl, bool use_linefitting_data)
{
    Eigen::Quaterniond q(Tcl.block<3,3>(0,0));
