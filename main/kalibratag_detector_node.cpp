...
public:
    kalibraTagDetector(ros::NodeHandle& nh, ros::NodeHandle& pnh, std::string config_file);
    ~kalibraTagDetector();
//    bool initParam(std::string filename);
    void imageCb(const sensor_msgs::ImageConstPtr& msg);    //callback function
...
kalibraTagDetector::kalibraTagDetector(ros::NodeHandle& nh, ros::NodeHandle& pnh, std::string config_file): it_(nh){
    ...
    std::cout <<"tag_space: "<< tagspace <<" black_border: "<<black_border<<std::endl;
    ...
    std::string img_topic_name = "image";   //default
...
//get config file and set params
  std::string config_file;
  config_file = readParam<std::string>(pnh, "config_file");
  readParameters(config_file);
  std::cout<<"image_topic : "<<img_topic_name<<std::endl;

  rosbag::Bag bag_input;
  bag_input.open(bag_path, rosbag::bagmode::Read);
  std::vector<std::string> topics;
  //scan_topic_name and img_topic_name  are included in src/config.cpp . they are global variable
  topics.push_back(scan_topic_name);
  topics.push_back(img_topic_name);
  rosbag::View views(bag_input, rosbag::TopicQuery(topics));

  // 初始视觉 pose
  kalibraTagDetector detector(nh, pnh, config_file);
